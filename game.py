...
def hero_movement_wasd():
    ...
    if game_map[hero_coordinates[0] + x][hero_coordinates[1] + y] == stone_picture:
        return its_a_stone_print()

    elif game_map[hero_coordinates[0] + x][hero_coordinates[1] + y] == monster_picture:
        monster_coordinates = [hero_coordinates[0] - 1, hero_coordinates[1]]
        return monster(monster_coordinates)
        
    elif game_map[hero_coordinates[0] + x][hero_coordinates[1] + y] == shop_picture:
        return shop_print()

    else:
        change_H_into_none()
        if game_map[hero_coordinates[0] + x][hero_coordinates[1] + y] == portal_picture_forward:
            if not tavern_flag:
                map_id = str(int(map_id) + 1)
            else:
                tavern_flag = False
            load_map_plus()
        elif game_map[hero_coordinates[0] + x][hero_coordinates[1] + y] == portal_picture_backward:
            if not tavern_flag:
                map_id = str(int(map_id) - 1)
            else:
                tavern_flag = False
            ...
            if game_map[hero_coordinates[0] + x][hero_coordinates[1] + y] == chest_picture:
                chest()
            elif game_map[hero_coordinates[0] + x][hero_coordinates[1] + y] == heart_picture:
                heart()
...
def hero_movement_chess():
    ...
    if 0 <= ord(word.split()[0]) - 65 < len(game_map) and 0 <= ord(word.split()[1]) - 65 < len(game_map[0]):

        if game_map[ord(word.split()[0]) - 65][ord(word.split()[1]) - 65] == stone_picture:
            return its_a_stone_print()
            
        elif game_map[ord(word.split()[0]) - 65][ord(word.split()[1]) - 65] == monster_picture:
            monster_coordinates = [ord(word.split()[0]) - 65, ord(word.split()[1]) - 65]
            return monster(monster_coordinates)
            
        elif game_map[ord(word.split()[0]) - 65][ord(word.split()[1]) - 65] == shop_picture:
            return shop_print()
            
        else:
            change_H_into_none()
            if game_map[ord(word.split()[0]) - 65][ord(word.split()[1]) - 65] == portal_picture_forward:
                if not tavern_flag:
                    map_id = str(int(map_id) + 1)
                else:
                    tavern_flag = False
                load_map_plus()
            elif game_map[ord(word.split()[0]) - 65][ord(word.split()[1]) - 65] == portal_picture_backward:
                if not tavern_flag:
                    map_id = str(int(map_id) - 1)
                else:
                    tavern_flag = False
                ...
                hero_coordinates[0] = ord(word.split()[0]) - 65
                hero_coordinates[1] = ord(word.split()[1]) - 65

                if game_map[ord(word.split()[0]) - 65][ord(word.split()[1]) - 65] == chest_picture:
                    chest()
                elif game_map[ord(word.split()[0]) - 65][ord(word.split()[1]) - 65] == heart_picture:
                    heart()
...
def monster(monster_coordinates):
    ...
    if choice == '1':
        print('Get ready!')
        print(f'You will fight {favorite_weapon}')
        print_time_sleep()
